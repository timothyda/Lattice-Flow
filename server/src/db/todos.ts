...
export function toggleTodo(id: number): Todo | undefined {
  ...
  const newStatus: TaskStatus = todo.task_status === 'complete' ? 'planning' : 'complete'
...
export function updateTaskStatus(todoId: number, newStatus: TaskStatus, actingUserId?: number): Todo | undefined {
  const db = getDb()
  ...
  db.prepare(`
    UPDATE todos SET task_status = ?, completed = ?, completed_at = ?, updated_at = ? WHERE id = ?
