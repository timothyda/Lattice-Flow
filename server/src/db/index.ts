...
function runAlterMigrations(db: Database.Database): void {
  ...
  if (!projCols.includes('due_date'))     db.exec('ALTER TABLE projects ADD COLUMN due_date TEXT')
  if (!projCols.includes('archived_at'))  db.exec('ALTER TABLE projects ADD COLUMN archived_at TEXT')

  const clientCols = cols(db, 'clients')
  if (!clientCols.includes('status'))       db.exec("ALTER TABLE clients ADD COLUMN status TEXT NOT NULL DEFAULT 'active'")
  if (!clientCols.includes('archived_at'))  db.exec('ALTER TABLE clients ADD COLUMN archived_at TEXT')
  if (!clientCols.includes('description'))  db.exec('ALTER TABLE clients ADD COLUMN description TEXT')
  if (!clientCols.includes('logo_url'))     db.exec('ALTER TABLE clients ADD COLUMN logo_url TEXT')
  if (!clientCols.includes('contact_role')) db.exec('ALTER TABLE clients ADD COLUMN contact_role TEXT')

  const todoCols = cols(db, 'todos')
  if (!todoCols.includes('task_status'))   db.exec("ALTER TABLE todos ADD COLUMN task_status TEXT NOT NULL DEFAULT 'planning'")
  if (!todoCols.includes('updated_at'))    db.exec("ALTER TABLE todos ADD COLUMN updated_at TEXT NOT NULL DEFAULT (datetime('now'))")
  if (!todoCols.includes('assigned_to'))   db.exec('ALTER TABLE todos ADD COLUMN assigned_to INTEGER REFERENCES users(id) ON DELETE SET NULL')
  if (!todoCols.includes('priority'))      db.exec("ALTER TABLE todos ADD COLUMN priority TEXT NOT NULL DEFAULT 'normal'")
  ...
  if (!todoCols.includes('description'))   db.exec("ALTER TABLE todos ADD COLUMN description TEXT NOT NULL DEFAULT ''")

  const userCols = cols(db, 'users')
  if (!userCols.includes('password_hash'))      db.exec('ALTER TABLE users ADD COLUMN password_hash TEXT')
  if (!userCols.includes('invite_token'))       db.exec('ALTER TABLE users ADD COLUMN invite_token TEXT')
  if (!userCols.includes('invite_expires'))     db.exec('ALTER TABLE users ADD COLUMN invite_expires TEXT')
  if (!userCols.includes('must_set_password'))  db.exec('ALTER TABLE users ADD COLUMN must_set_password INTEGER NOT NULL DEFAULT 0')
  if (!userCols.includes('failed_attempts'))    db.exec('ALTER TABLE users ADD COLUMN failed_attempts INTEGER NOT NULL DEFAULT 0')
  ...
  if (!userCols.includes('recovery_code_hash')) db.exec('ALTER TABLE users ADD COLUMN recovery_code_hash TEXT')

  const sessionCols = cols(db, 'time_sessions')
  if (!sessionCols.includes('todo_id'))       db.exec('ALTER TABLE time_sessions ADD COLUMN todo_id INTEGER REFERENCES todos(id) ON DELETE SET NULL')
  if (!sessionCols.includes('subtask_title')) db.exec('ALTER TABLE time_sessions ADD COLUMN subtask_title TEXT')

  const todoCols2 = cols(db, 'todos')
  if (!todoCols2.includes('task_template_id')) db.exec('ALTER TABLE todos ADD COLUMN task_template_id INTEGER REFERENCES task_templates(id) ON DELETE SET NULL')

  const projCols2 = cols(db, 'projects')
  if (!projCols2.includes('project_type')) db.exec("ALTER TABLE projects ADD COLUMN project_type TEXT NOT NULL DEFAULT 'other'")

  const todoCols3 = cols(db, 'todos')
  if (!todoCols3.includes('is_recurring'))           db.exec('ALTER TABLE todos ADD COLUMN is_recurring INTEGER NOT NULL DEFAULT 0')
  if (!todoCols3.includes('recurrence_frequency'))   db.exec('ALTER TABLE todos ADD COLUMN recurrence_frequency TEXT')
  if (!todoCols3.includes('recurrence_next_date'))   db.exec('ALTER TABLE todos ADD COLUMN recurrence_next_date TEXT')
