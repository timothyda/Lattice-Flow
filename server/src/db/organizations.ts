...
export function getOrganization(): Organization | undefined {
  return getDb().prepare('SELECT * FROM organizations LIMIT 1').get() as Organization | undefined
}
...
export function createOrganization(data: NewOrganization): Organization {
  const { lastInsertRowid } = getDb()
    ...
    .run(data.name)
  return getDb().prepare('SELECT * FROM organizations WHERE id = ?').get(Number(lastInsertRowid)) as Organization
}
...
export function organizationExists(): boolean {
  const row = getDb().prepare('SELECT COUNT(*) as count FROM organizations').get() as { count: number }
  return row.count > 0
}
