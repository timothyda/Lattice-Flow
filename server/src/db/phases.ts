...
export function getPhase(id: number): Phase | undefined {
  return getDb().prepare('SELECT * FROM phases WHERE id = ?').get(id) as Phase | undefined
}
...
export function createPhase(data: NewPhase): Phase {
  const { lastInsertRowid } = getDb()
    .prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `)
    .run(data.project_id, data.name, data.order, data.description ?? '', data.planned_start ?? null, data.planned_end ?? null, data.status ?? 'planning')
...
export function updatePhase(id: number, data: UpdatePhase): Phase | undefined {
  ...
  getDb().prepare(`UPDATE phases SET ${updates.join(', ')} WHERE id = ?`).run(...values, id)
  return getPhase(id)
}

export function deletePhase(id: number): boolean {
  const { changes } = getDb().prepare('DELETE FROM phases WHERE id = ?').run(id)
  return changes > 0
}
