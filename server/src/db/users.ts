...
export function updateUser(id: number, data: Partial<Pick<User, 'role' | 'dashboard_widgets'>>): User | undefined {
  ...
  if ('dashboard_widgets' in data && data.dashboard_widgets !== undefined)     { fields.push('dashboard_widgets = ?'); values.push(data.dashboard_widgets) }
...
export function setPasswordHash(userId: number, hash: string): void {
  getDb().prepare('UPDATE users SET password_hash = ?, must_set_password = 0 WHERE id = ?').run(hash, userId)
}

export function generateInviteToken(userId: number): string {
  const token = crypto.randomBytes(16).toString('hex')
  const expires = new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString()
  getDb().prepare('UPDATE users SET invite_token = ?, invite_expires = ?, must_set_password = 1 WHERE id = ?')
