...
const UPDATABLE: (keyof UpdateMeeting)[] = ['title', 'date', 'recording_path', 'calendar_event_id', 'notes']
...
export function createMeeting(data: NewMeeting): Meeting {
  const { lastInsertRowid } = getDb()
    .prepare('INSERT INTO meetings (project_id, title, date, recording_path, calendar_event_id) VALUES (?, ?, ?, ?, ?)')
    .run(data.project_id, data.title, data.date, data.recording_path ?? null, data.calendar_event_id ?? null)
...
export function deleteMeeting(id: number): boolean {
  const { changes } = getDb().prepare('DELETE FROM meetings WHERE id = ?').run(id)
  return changes > 0
}
