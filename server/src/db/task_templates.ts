...
export function seedDefaultTemplates(orgId: number): void {
  const db = getDb()
  const existing = db.prepare('SELECT COUNT(*) as n FROM task_templates WHERE organization_id = ?').get(orgId) as { n: number }
  if (existing.n > 0) return

  const insertTemplate = db.prepare('INSERT INTO task_templates (organization_id, project_type, title, sort_order) VALUES (?, ?, ?, ?)')
  const insertSubtask = db.prepare('INSERT INTO task_template_subtasks (task_template_id, title, sort_order) VALUES (?, ?, ?)')
...
function attachSubtasks(templates: Omit<TaskTemplate, 'subtasks'>[]): TaskTemplate[] {
  ...
  return templates.map((t) => ({
    ...t,
    subtasks: db.prepare('SELECT * FROM task_template_subtasks WHERE task_template_id = ? ORDER BY sort_order ASC, id ASC')
...
export function getSubtitlesForTodoByStatus(todoId: number, status: string): string[] {
  ...
  if (hint) {
    const db = getDb()
    const proj = db.prepare('SELECT p.organization_id, p.project_type FROM todos t JOIN projects p ON p.id = t.project_id WHERE t.id = ?')
      .get(todoId) as { organization_id: number; project_type: string } | undefined
    if (proj) {
      const tpl = db.prepare('SELECT id FROM task_templates WHERE organization_id = ? AND project_type = ? AND title = ? COLLATE NOCASE LIMIT 1')
        .get(proj.organization_id, proj.project_type, hint) as { id: number } | undefined
      if (tpl) {
        return (db.prepare('SELECT title FROM task_template_subtasks WHERE task_template_id = ? ORDER BY sort_order ASC, id ASC')
...
export function getSubtitlesForTodo(todoId: number): string[] {
  ...
  const row = db.prepare(`
    ...
    FROM todos t JOIN projects p ON p.id = t.project_id WHERE t.id = ?
  ...
  if (!templateId) {
    const match = db.prepare('SELECT id FROM task_templates WHERE organization_id = ? AND project_type = ? AND title = ? COLLATE NOCASE LIMIT 1')
  ...
  return (db.prepare('SELECT title FROM task_template_subtasks WHERE task_template_id = ? ORDER BY sort_order ASC, id ASC')
    .all(templateId) as { title: string }[]).map((r) => r.title)
}

export function addSubtask(templateId: number, title: string): TaskTemplateSubtask {
  const db = getDb()
  const maxOrder = (db.prepare('SELECT COALESCE(MAX(sort_order), -1) as m FROM task_template_subtasks WHERE task_template_id = ?').get(templateId) as { m: number }).m
  const { lastInsertRowid } = db.prepare('INSERT INTO task_template_subtasks (task_template_id, title, sort_order) VALUES (?, ?, ?)').run(templateId, title.trim(), maxOrder + 1)
  return db.prepare('SELECT * FROM task_template_subtasks WHERE id = ?').get(Number(lastInsertRowid)) as TaskTemplateSubtask
}
...
export function addTemplate(orgId: number, projectType: ProjectType, title: string): TaskTemplate {
  const db = getDb()
  const maxOrder = (db.prepare('SELECT COALESCE(MAX(sort_order), -1) as m FROM task_templates WHERE organization_id = ? AND project_type = ?').get(orgId, projectType) as { m: number }).m
  const { lastInsertRowid } = db.prepare('INSERT INTO task_templates (organization_id, project_type, title, sort_order) VALUES (?, ?, ?, ?)').run(orgId, projectType, title.trim(), maxOrder + 1)
