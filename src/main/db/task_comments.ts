...
export function listComments(todoId: number): TaskComment[] {
  return getDb()
    .prepare('SELECT * FROM task_comments WHERE todo_id = ? ORDER BY created_at ASC')
    .all(todoId) as TaskComment[]
}
...
export function addComment(
  ...
  if (orgId) {
    const orgUsers = db.prepare(
      'SELECT id, display_name FROM users WHERE organization_id = ?'
    ).all(orgId) as { id: number; display_name: string }[]

    const todo = db.prepare(`
      SELECT t.title, p.name as project_name
      FROM todos t JOIN projects p ON t.project_id = p.id WHERE t.id = ?
...
export function getUnreadCountsForTodos(
  todoIds: number[],
  userId: number
