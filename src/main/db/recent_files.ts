...
export function recordFileAccess(userId: number, projectId: number, filePath: string, fileName: string): void {
  const db = getDb()
  // Remove existing entry for same file/user to avoid duplicates
  db.prepare('DELETE FROM recent_files WHERE user_id = ? AND file_path = ?').run(userId, filePath)
  db.prepare('INSERT INTO recent_files (user_id, project_id, file_path, file_name) VALUES (?, ?, ?, ?)')
    .run(userId, projectId, filePath, fileName)
