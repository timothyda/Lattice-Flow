...
function getSession(id: number): TimeSession | undefined {
  return getDb()
    .prepare(`${SESSION_SELECT} WHERE ts.id = ?`)
    .get(id) as TimeSession | undefined
}
...
export function clockIn(projectId: number, userId: number, todoId?: number | null): TimeSession {
  ...
  const { lastInsertRowid } = getDb()
    .prepare(`
      INSERT INTO time_sessions (project_id, user_id, user_name, todo_id, started_at)
      VALUES (?, ?, ?, ?, datetime('now'))
    `)
  ...
  const { lastInsertRowid } = getDb()
    .prepare(`
      INSERT INTO time_sessions
        (project_id, user_id, user_name, todo_id, subtask_title, started_at, ended_at, duration_minutes, note)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `)
    .run(
      data.project_id,
...
export function exportSessionsCSV(projectId: number): string {
  ...
  const toLocalDate = (utc: string) =>
    new Date(utc.replace(' ', 'T') + 'Z').toLocaleDateString('en-CA')
  ...
  const lines = rows.map((r) => [
    toLocalDate(r.started_at),
    escape(r.user_name),
    escape(r.email),
    escape(r.todo_title),
    escape(r.subtask_title),
    toLocalTime(r.started_at),
    toLocalTime(r.ended_at),
