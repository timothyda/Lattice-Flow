...
// The statuses that are configurable for routing (planning/in_progress/complete are handled specially)
...
export function seedDefaultRouting(orgId: number): void {
  const db = getDb()
  const { c } = db.prepare('SELECT COUNT(*) as c FROM role_routing WHERE organization_id = ?')
    .get(orgId) as { c: number }
  if (c > 0) return

  const insert = db.prepare(
    'INSERT OR IGNORE INTO role_routing (organization_id, role, task_status) VALUES (?, ?, ?)'
  )
...
// Returns routing as status -> roles[] (for use in routeTask)
export function getOrgRouting(orgId: number): Partial<Record<TaskStatus, UserRole[]>> {
  const rows = getDb().prepare(
    'SELECT role, task_status FROM role_routing WHERE organization_id = ?'
...
// Returns routing inverted as role -> statuses[] (for the settings UI)
...
// Replace which statuses route to a given role for an org
export function setRoleRouting(orgId: number, role: UserRole, statuses: TaskStatus[]): void {
  const db = getDb()

  // Seed defaults first if this org has no routing config yet
  const { c } = db.prepare('SELECT COUNT(*) as c FROM role_routing WHERE organization_id = ?')
    .get(orgId) as { c: number }
