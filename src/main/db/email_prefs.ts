...
export function listClientContacts(clientId: number): ClientContact[] {
  return getDb()
    .prepare('SELECT * FROM client_contacts WHERE client_id = ? ORDER BY name ASC')
    .all(clientId) as ClientContact[]
}
...
export function upsertProjectClientNotification(
  ...
  `).run(
    projectId,
    clientContactId,
    settings.status_update ? 1 : 0,
    settings.new_file ? 1 : 0,
    settings.meeting_scheduled !== false ? 1 : 0,
  )
}

export function removeProjectClientNotification(projectId: number, clientContactId: number): void {
  getDb()
    .prepare('DELETE FROM project_client_notifications WHERE project_id = ? AND client_contact_id = ?')
    .run(projectId, clientContactId)
}

// Returns contacts for a client that are NOT yet added to a project's notification list
export function getAvailableContactsForProject(projectId: number, clientId: number): ClientContact[] {
  return getDb().prepare(`
    ...
    WHERE cc.client_id = ?
      AND cc.id NOT IN (
        SELECT client_contact_id FROM project_client_notifications WHERE project_id = ?
      )
    ORDER BY cc.name ASC
