...
export interface User {
  ...
  role: UserRole
  dashboard_widgets: string  // JSON array of widget IDs
  must_set_password: number  // 1 = invite pending, 0 = active
...
export interface ArchivedItem {
  ...
  id: number
  name: string
  parent_name: string | null   // client name for projects, null for clients
  archived_at: string
  organization_id: number
}
...
// Which roles receive the task when each status is set.
// Empty array = no auto-routing (keep existing or clear).
...
export interface TaskAssignment {
  ...
  todo_id: number
  user_id: number
  display_name: string | null  // joined
  assigned_at: string
}
...
export interface TimeSession {
  ...
  user_id: number
  user_name: string
  todo_id: number | null        // which task this session is for
  todo_title: string | null     // joined
...
export interface AppNotification {
  ...
  todo_id: number | null
  todo_title: string | null     // joined
  project_name: string | null   // joined
...
export interface ProjectClientNotification {
  ...
  contact_name: string | null    // joined
  contact_email: string | null   // joined
...
export type CalendarProvider = 'microsoft' | 'google' | 'zoom' | 'caldav'
